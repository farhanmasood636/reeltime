...
class LocationMapFormController extends Controller
{
    ...
    public function postCreateStepOne(Request $request)
    {
        $validatedData = $request->validate([
            ...
            "access_fee" => "required",
            "hot_link" => "required",
            "hot_link_image_video" =>
                "file|mimes:jpeg,png,jpg,gif,svg,mpeg,ogg,mp4,webm,3gp,mov,flv,avi,wmv,ts|max:100040",
        ]);
    ...
    public function postCreateStepTwo(Request $request)
    {
        $validatedData = $request->validate([
            ...
            "dock" => "required",
            "applicable_image_video_fishing" =>
                "file|mimes:jpeg,png,jpg,gif,svg,mpeg,ogg,mp4,webm,3gp,mov,flv,avi,wmv,ts|max:100040",
        ]);
        ...
        Session::put([
            ...
            "applicable_image_video_fishing" =>
                $input["applicable_image_video_fishing"],
    ...
    public function postCreateStepThree(Request $request)
    {
        $validatedData = $request->validate([
            ...
            "applicable_image_video_parking" =>
                "file|mimes:jpeg,png,jpg,gif,svg,mpeg,ogg,mp4,webm,3gp,mov,flv,avi,wmv,ts|max:100040",
        ]);
        ...
        Session::put([
            ...
            "applicable_image_video_parking" =>
                $input["applicable_image_video_parking"],
        ]);
    ...
    public function postCreateStepFour(Request $request)
    {
        ...
        $input["sunrise_to_sunset_access"] = $request->input(
            "sunrise_to_sunset_access"
        );

        Session::put([
            "access_24" => $request->access_24,
            "sunrise_to_sunset_access" => $input["sunrise_to_sunset_access"],
            "specify_sunrise_to_sunset_access" =>
                $request->specify_sunrise_to_sunset_access,
            ...
            "often_hours_of_use_specify" =>
                $request->often_hours_of_use_specify,
            "applicable_link_hours" => $request->applicable_link_hours,
            "applicable_image_video_hours" =>
                $input["applicable_image_video_hours"],
        ]);
    ...
    public function postCreateStepFive(Request $request)
    {
        ...
        Session::put([
            ...
            "ramp_surface_other_specify" =>
                $request->ramp_surface_other_specify,
            "restrooms" => $request->restrooms,
            "applicable_link_boat" => $request->applicable_link_boat,
            "applicable_image_video_boat" =>
                $input["applicable_image_video_boat"],
    ...
    public function postCreateStepSix(Request $request)
    {
        ...
        $input["allowed_camping_type"] = $request->input(
            "allowed_camping_type"
        );
        $input["picnic_area_amenities"] = $request->input(
            "picnic_area_amenities"
        );
        $input["family_area_recreation_type"] = $request->input(
        ...
        Session::put([
            ...
            "family_area_recreation_type" =>
                $input["family_area_recreation_type"],
        ...
        $LocationMap->common_species_harvested =
            $allSessions["common_species_harvested"];
        ...
        $LocationMap->hot_link_image_video =
            $allSessions["hot_link_image_video"];
        ...
        $LocationMap->description_of_attractor =
            $allSessions["description_of_attractor"];
        $LocationMap->species = $allSessions["species"];
        $LocationMap->fish_cleaning_station =
            $allSessions["fish_cleaning_station"];
        $LocationMap->dock = $allSessions["dock"];
        $LocationMap->courtesy_dock_numbers =
            $allSessions["courtesy_dock_numbers"];
        $LocationMap->applicable_link_fishing =
            $allSessions["applicable_link_fishing"];
        $LocationMap->applicable_image_video_fishing =
            $allSessions["applicable_image_video_fishing"];
        ...
        $LocationMap->trailer_spaces_numbers =
            $allSessions["trailer_spaces_numbers"];
        $LocationMap->handicap_spaces = $allSessions["handicap_spaces"];
        $LocationMap->handicap_spaces_numbers =
            $allSessions["handicap_spaces_numbers"];
        $LocationMap->lighted_parking = $allSessions["lighted_parking"];
        $LocationMap->lighted_parking_type =
            $allSessions["lighted_parking_type"];
        $LocationMap->applicable_link_parking =
            $allSessions["applicable_link_parking"];
        $LocationMap->applicable_image_video_parking =
            $allSessions["applicable_image_video_parking"];

        // Hours Form Session
        $LocationMap->access_24 = $allSessions["access_24"];
        $LocationMap->sunrise_to_sunset_access = implode(
            " ",
            $allSessions["sunrise_to_sunset_access"]
        );
        $LocationMap->specify_sunrise_to_sunset_access =
            $allSessions["specify_sunrise_to_sunset_access"];
        $LocationMap->open_year_round = $allSessions["open_year_round"];
        $LocationMap->open_year_round_specify =
            $allSessions["open_year_round_specify"];
        $LocationMap->closed_months = $allSessions["closed_months"];
        $LocationMap->closed_months_specify =
            $allSessions["closed_months_specify"];
        $LocationMap->often_hours_of_use = $allSessions["often_hours_of_use"];
        $LocationMap->often_hours_of_use_specify =
            $allSessions["often_hours_of_use_specify"];
        $LocationMap->applicable_link_hours =
            $allSessions["applicable_link_hours"];
        $LocationMap->applicable_image_video_hours =
            $allSessions["applicable_image_video_hours"];

        // Boat Form Session
        $LocationMap->number_of_ramps = implode(
            " ",
            $allSessions["number_of_ramps"]
        );
        $LocationMap->number_of_lanes = implode(
            " ",
            $allSessions["number_of_lanes"]
        );
        $LocationMap->ramp_surface = implode(" ", $allSessions["ramp_surface"]);
        $LocationMap->ramp_surface_other_specify =
            $allSessions["ramp_surface_other_specify"];
        $LocationMap->restrooms = $allSessions["restrooms"];
        $LocationMap->applicable_link_boat =
            $allSessions["applicable_link_boat"];
        $LocationMap->applicable_image_video_boat =
            $allSessions["applicable_image_video_boat"];
        $LocationMap->boat_ramp_access_rating =
            $allSessions["boat_ramp_access_rating"];

        // Area Form Session
        $LocationMap->is_camping_allowed = $allSessions["is_camping_allowed"];
        $LocationMap->allowed_camping_type = implode(
            " ",
            $allSessions["allowed_camping_type"]
        );
        ...
        $LocationMap->public_area_amenities = implode(
            " ",
            $allSessions["picnic_area_amenities"]
        );
        $LocationMap->family_area_recreation =
            $allSessions["family_area_recreation"];
        $LocationMap->family_area_recreation_type = implode(
            " ",
            $allSessions["family_area_recreation_type"]
        );
        $LocationMap->area_amenities_rating =
            $allSessions["area_amenities_rating"];
